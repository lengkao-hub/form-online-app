      ...
      <div className="w-full space-y-3">
        {value.length < maxFiles && (
          <div
            {...getRootProps()}
            className={cn(
              "border-2 border-dashed rounded-lg flex items-center justify-center cursor-pointer transition-colors p-1.5",
              isDragActive ? "border-primary bg-primary/10" : "border-gray-200 hover:border-primary"
            )}
          >
            <input {...getInputProps()} />
            <p className="text-center text-sm text-gray-500">ເລືອກໄຟລ໌</p>
          </div>
        )}
         ...
         {value.map((file: any, index: number) => (
          <Card
            key={index}
            className="relative  h-[40px] flex items-center  border border-gray-300 p-4 group cursor-pointer"
          ...
          >
            ...
            <Button
              ...
              variant="destructive"
              size="icon"
              className="absolute rounded-[10px] top-[1px] right-[1px] opacity-0 group-hover:opacity-100 transition-opacity"
              onClick={(e) => {
                e.stopPropagation();
                removeFile(index);
              }}
