...
const fetchFolder = async ({ page, limit, search, excludeApplications, genderFilter, yearFilter, dateFilter, officeId, status }: {
  page: number;
  limit: number;
  officeId?: number;
  search: string;
...
const useTable = ({ excludeApplications = false, officeIds }: { excludeApplications?: boolean, officeIds?: string }) => {
  ...
  const query = useQuery<IProfileResponse, Error>({
    queryKey: ["approved-user", page, limit, debouncedSearch, excludeApplications, genderFilter, dateFilter, yearFilter, status],
    queryFn: async () => await fetchFolder({ page, limit, search: debouncedSearch, excludeApplications, genderFilter, yearFilter, dateFilter, status }),
