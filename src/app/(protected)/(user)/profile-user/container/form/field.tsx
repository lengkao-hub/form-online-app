...
export const PersonalInfoSection: React.FC<IFormProps & {
  ...
  useUpdateDefaultValues({ form, fieldName: "nationalityId", value: 156, shouldUpdate: true });
  if (action) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-4">
        ...
        <Form.Field name="identityExpiryDate" control={form.control} label="ວັນຫມົດອາຍຸ" required={false}>
          <Form.Input.DateTimePicker />
        </Form.Field>
      </div>
    </div>
...
export const CurrentAddressSection: React.FC<IFormDistrictProps> = ({ form, setIsAddingVillage, formRef }) => {
  ...
  useUpdateDefaultValues({ form, fieldName: "currentProvince", value: 1, shouldUpdate: true });
  useUpdateDefaultValues({ form, fieldName: "currentDistrict", value: 101, shouldUpdate: true });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Form.Field name="currentProvince" control={form.control} label="ແຂວງ" required={false}>
          <Form.Input.Combobox placeholder="ເລືອກແຂວງ" className="w-full" options={provinceOptions} formRef={formRef} />
        </Form.Field>
        <Form.Field name="currentDistrict" control={form.control} label="ເມືອງ" required={false}>
          <Form.Input.Combobox placeholder="ເລືອກເມືອງ" className="w-full" options={districtOptions} formRef={formRef} />
        </Form.Field>
        <Form.Field name="currentVillageId" control={form.control} label="ບ້ານ" required={false}>
...
export const OverseasAddressSection: React.FC<IFormProps> = ({ form, formRef }) => {
  const { result: ethnicityOptions } = useNationalitiesCombobox({ isNationality: true });
  useUpdateDefaultValues({ form, fieldName: "overseasCountryId", value: 156, shouldUpdate: true });
  return (
    <div className="space-y-4">
