...
interface IFolderCardList {
    result: IFolder[],
    loading: any,
    showedit: boolean,

}
export default function FolderCardList({ result, loading, showedit }: IFolderCardList) {
  ...
  const handleReject = () => {
    editStatus({ id: selectedId, status: "REJECTED", reason })
    setOpen(false)
    setReason("")
  }

  const detailsAdmin = (id: number, content: string, showedit: boolean) => {
    router.push(`profile/details-folder/${id}?showedit=${showedit}&content=${content}`);
  }
  const detailsUser = (id: number, content: string, showedit: boolean) => {
    router.push(`rejected-profile/details-folder/${id}?showedit=${showedit}&content=${content}`);
  }
  const handleDetailsClick = (id: number, content: string, showedit: boolean) => {
    if (role === "ADMIN") {
      detailsAdmin(id, content, showedit);
    } else if (role === "USER") {
      detailsUser(id, content, showedit);
    } else {
    ...
    <>
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {result.map((item) => {
          ...
          const folderPrice = item.folderPrice
          return (
            <Card
              key={item.id}
            ...
            >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <div
                          ...
                          onClick={() => handleDetailsClick(item.id, item.content, showedit)}
                        >
                          {showedit ? "ແກ້ໄຂ" : "ລາຍລະອຽດ"}
                        </div>
                      </div>
                  ...
                  {Array.isArray(profile) && profile.length > 0 ? (
                    <>
                      <div className="max-h-[100px] overflow-y-auto space-y-3">
                        {folderPrice.map((item, index) => (
                          <div
                            key={index}
                            className="flex justify-between items-center bg-white shadow-md p-4 rounded-lg mb-3"
                          >
                            <div className="text-gray-700 font-medium">
                                                            ຈຳນວນ: <span>{item.amount}</span>
                            </div>
                            <div className="text-gray-700 font-medium">
                                                            ລາຄາ: <span>{item.price} </span>
                            </div>
                            <div className="text-gray-700 font-bold">
                                                            ລວມທັງໝົດ: <span>{item.totalPrice} </span>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              {/* ✅ ປຸ້ມຢູ່ລຸ່ມສຸດ */}
              {showedit ? (
                <div className="flex justify-end gap-3 p-4 mt-auto text-red-500 ">
                  {item.content}
                </div>
              )
                : (
                  <div className="flex justify-end gap-3 p-4 mt-auto ">
                    <Button
                      ...
                      onClick={() => editStatus({ id: item.id, status: "APPROVED" })}
                    >
                                            ອະນຸມັດ
                    </Button>
