...
const fetchVillage = async ({ page, limit, search }: {
  ...
  const response = await apiClient.get<IVisaResponse>("/blacklist-type", {
    params,
  });
  return response;
};
...
const useTypeTable = () => {
  ...
  const query = useQuery<IVisaResponse, Error>({
    queryKey: ["types", page, limit, debouncedSearch, statusFilter, districtId],
